...
public class TwitchClient {
  ...
  private static final String TOP_GAME_URL_TEMPLATE =
  ...
  private static final String STREAM_SEARCH_URL_TEMPLATE =
      "https://api.twitch.tv/helix/streams?game_id=%s&first=%s";
  private static final String VIDEO_SEARCH_URL_TEMPLATE =
      "https://api.twitch.tv/helix/videos?game_id=%s&first=%s";
  private static final String CLIP_SEARCH_URL_TEMPLATE =
      "https://api.twitch.tv/helix/clips?game_id=%s&first=%s";
  ...
  // send different url template
  private String buildSearchURL(String url, String gameId, int limit) {
    try {
      // sent from front-end
      // even though might not have space
      // still need be careful with since cannot verify correctness of format
  ...
  private String searchTwitch(String url) throws TwitchException {
    ...
    try {
      ...
      e.printStackTrace();
      throw new TwitchException("Failed to retrieve Twitch token and clientID");
    }

    HttpGet request = new HttpGet(url);
    request.setHeader("Authorization", prop.getProperty("token"));
    request.setHeader("Client-Id", prop.getProperty("client_id"));
    try {
  ...
  private List<Item> searchStreams(String gameId, int limit) throws TwitchException {
    ...
    List<Item> streams =
        getItemList(searchTwitch(buildSearchURL(STREAM_SEARCH_URL_TEMPLATE, gameId, limit)));
    for (Item item : streams) {
      // we already have others in Twitch response
      // but don't have url and type
      // broadcaster name is the same as user name
  ...
  private List<Item> searchVideos(String gameId, int limit) throws TwitchException {
    List<Item> videos =
        getItemList(searchTwitch(buildSearchURL(VIDEO_SEARCH_URL_TEMPLATE, gameId, limit)));
  ...
  private List<Item> searchClips(String gameId, int limit) throws TwitchException {
    List<Item> clips =
        getItemList(searchTwitch(buildSearchURL(CLIP_SEARCH_URL_TEMPLATE, gameId, limit)));
    for (Item item : clips) {
  ...
  public List<Item> searchByType(String gameId, ItemType type, int limit) throws TwitchException {
    ...
    // not all Twitch response has game_id
    for (Item item : items) {
